    ...
    with modification_container:
        ...
        for column in to_filter_columns:
            ...
            if is_categorical_dtype(df[column]) or df[column].nunique() < 20:
                ...
                df =df[df.apply(lambda x: x.astype(str).str.contains(user_cat_input).any(), axis=1)].copy()
                ...
                if user_text_input:
                    df =df[df.apply(lambda x: x.astype(str).str.contains(user_cat_input).any(), axis=1)].copy()
                    df = df.applymap(str).style.applymap(lambda x: "background: #ccebc4" if re.search(user_cat_input, x) else "")
